$(document).ready(function(){
    ...
    $(document).on('click', '.editSemester', function () {
      ...
      if ($(`#isRecent${id}`).html() == 'true') {
        $("form#addEditSemesterForm select#recentType option[value='true']").attr("selected", true);
      }
      else {
        $("form#addEditSemesterForm select#recentType option[value='false']").attr("selected", true);
      }
...
function createSemesterRow(semesterInfo, table) {
        ...
        semesterOutput += "<td> <span class='editSemester glyphicon glyphicon-pencil'></span> <span class='deleteSemester glyphicon glyphicon-trash'></span> </td>";
        semesterOutput += "</tr>";
