...
usersRouter.post("/validateLogin", function (req, res) {
  ...
  dbConnection.query(sql, [username, hashed_pwd], function (
    err,
    result,
    fields
  ) {
...
usersRouter.post("/editUser", function (req, res) {
  ...
  dbConnection.query(
    loginSql,
    [req.body.login, parseInt(req.body.userId)],
    function (err, result, fields) {
      if (err) throw error;
      // Login is already in table
      if (result.length !== 0) {
        res.json({ isUserEdited: false });
      } else {
...
usersRouter.post("/addUser", function (req, res) {
  ...
  dbConnection.query(loginSql, [req.body.login], function (
    err,
    result,
    fields
  ) {
...
// Don't delete user if it is the currently logged in user or 
// the user has already registered for classes
// Send back which error to show; '' for no error
usersRouter.post("/deleteUser", function (req, res) {
  // Delete the user with the specified ID
  if (req.session.userId != req.body.id) {
    var checkUserUsageSql = "SELECT * FROM Registrations WHERE StudentID = ?";
    var args = [req.body.id];
    dbConnection.query(checkUserUsageSql, args, function (err, result, fields) {
      if (result.length != 0) {
        res.json({ deletionError: 'registeredUser' });
      } else {
        var deleteUserSql = "DELETE FROM Users WHERE UserID = ?";
        dbConnection.query(deleteUserSql, args, function (
          err,
          result,
          fields
        ) {
          res.json({ deletionError: '' });
        });
      }
    });
  }
  // Error if trying to delete currently logged in user
    ...
    res.json({ deletionError: 'currentUser' });
  }
});
